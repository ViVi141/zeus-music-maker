...
mod tests {
    ...
    #[test]
    fn test_chunk_config_adjustment() {
        let mut config = VideoChunkConfig::default();
        
        // 测试短视频
        let short_video = VideoInfo { duration: 90, resolution: (1920, 1080) };
        config.adjust_for_video(&short_video);
        assert_eq!(config.max_chunks, 1);
        
        // 测试中等视频
        let medium_video = VideoInfo { duration: 300, resolution: (1920, 1080) };
        config.adjust_for_video(&medium_video);
        assert!(config.max_chunks >= 2 && config.max_chunks <= 4);
        
        // 测试长视频
        let long_video = VideoInfo { duration: 1200, resolution: (1920, 1080) };
        config.adjust_for_video(&long_video);
        assert!(config.max_chunks >= 4 && config.max_chunks <= 8);
    }
    ...
    fn test_chunk_count_calculation() {
        ...
        assert_eq!(config.calculate_chunk_count(30), 1);   // 短于分片时长
        assert_eq!(config.calculate_chunk_count(60), 1);   // 等于分片时长
        assert_eq!(config.calculate_chunk_count(120), 2);  // 2个分片
        assert_eq!(config.calculate_chunk_count(600), 10); // 10个分片
    }

    #[test]
    fn test_duration_parsing() {
        // 这里需要创建一个转换器实例来测试
        // 由于需要FFmpeg路径，我们只测试逻辑部分
        assert_eq!(parse_duration_static("01:30:45"), 5445);
        assert_eq!(parse_duration_static("02:15:30.500"), 8130);
        assert_eq!(parse_duration_static("00:05:20"), 320);
    }

    fn parse_duration_static(duration_str: &str) -> u32 {
        let parts: Vec<&str> = duration_str.split(':').collect();
        if parts.len() == 3 {
            let hours: u32 = parts[0].parse().unwrap_or(0);
            let minutes: u32 = parts[1].parse().unwrap_or(0);
